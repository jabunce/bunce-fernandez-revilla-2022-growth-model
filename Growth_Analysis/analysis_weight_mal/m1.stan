...
data {
  ...
  //int<lower=0> P;               // number of people measured
  vector<lower=0>[N] age;         // vector of ages for each observation (in years since conception)
  vector<lower=0>[N] weight;      // vector of observations (in kg)

  real<upper=0> meanLQ1;          // means from PriorPredictInitial
  ...
  real<lower=0> sdLQ1;             // stdevs from PriorPredictInitial
  real<lower=0> sdLQ2;
  real<lower=0> sdLQ3;
  real<lower=0> sdLK1;
  real<lower=0> sdLK2;
...
model {
  ...
  for (n in 1:N) {
    ...
    mu[n] = ( -1*exp(-1*muK1*muQ1*age[n]/( 1 + 2*muQ1 )) + 2*muH1/muK1*pi()^(muQ1/( 1 + 2*muQ1 )) )^(( 1 + 2*muQ1 )/muQ1) +
            ( -1*exp(-1*muK2*muQ2*age[n]/( 1 + 2*muQ2 )) + 2*muH2/muK2*pi()^(muQ2/( 1 + 2*muQ2 )) )^(( 1 + 2*muQ2 )/muQ2) +
            ( -1*exp(-1*muK3*muQ3*age[n]/( 1 + 2*muQ3 )) + 2*muH3/muK3*pi()^(muQ3/( 1 + 2*muQ3 )) )^(( 1 + 2*muQ3 )/muQ3);
