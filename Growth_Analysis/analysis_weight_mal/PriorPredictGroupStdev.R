...
for (n in 1:Nsims) {
  ...
  isLQ1 <- 0.0005     # stdevs for normal distribution
  isLQ2 <- 0.005        
  isLQ3 <- 0.005  #0.001
  isLK1 <- 0.0005       
  isLK2 <- 0.005  
...
pdf(file="./Plots/Prior_predict_m3_mw.pdf",
    height=10, width=10)
par(mfrow = c(2, 2))
par(mar = c(0, 0, 0, 0), oma = c(5, 5, 4, 4)) #margins for indiv plot, oma for outer margins (bottom, left, top, right)
  ...
  symbol.Male(centerx = 18, centery = 70, rayonx=0.5, lwd=2, col="black") # Male symbol from HelpersMG package
  ...
  symbol.Male(centerx = 20, centery = 70, rayonx=0.5, lwd=2, col="black") # Male symbol from HelpersMG package
