...
saveRDS(sitar_model, "sitar_model.RDS")
#plot(sitar.com.mal.9.e, subset = eth == 0)
#lines(sitar.com.mal.9.e, subset = eth == 1, col = 2, y2par = list(col = 2))


# reconstruct spline (using McElreath 2020 pg 117 and https://cran.r-project.org/web/packages/bsitar/vignettes/Bayesian_SITAR_model_An_Introduction.html)
# knots argument to SITAR: default 'df' quantiles of 'x' distribution
# xoffset argument to SITAR: optional value of offset for x (either "mean" (default), "apv" or value).
knot_list <- quantile( log(Com.mal.eth.e$TotAge) - mean(log(Com.mal.eth.e$TotAge)) , probs=seq(0,1,length.out=11) )
knot_list <- knot_list[2:10]
boundary_knot_list <- c(-2.757394, 1.217558) # copied from SITAR output
coefs <- sitar_model$coefficients$fixed
size_ber <- coefs["a"]
size_mat <- coefs["a"] + coefs["a.eth"] #+ random.effects(sitar_model)[203,"a"]
time_ber <- coefs["b"]
time_mat <- coefs["b"] + coefs["b.eth"] #+ random.effects(sitar_model)[203,"b"]
inte_ber <- coefs["c"]
inte_mat <- coefs["c"] + coefs["c.eth"] #+ random.effects(sitar_model)[203,"c"]

#decentered_log_TotAge_Ber <- log(Com.mal.eth.e[which(Com.mal.eth.e$eth==0),"TotAge"]) - mean( log(Com.mal.eth.e[which(Com.mal.eth.e$eth==0),"TotAge"]) )
#decentered_log_TotAge <- log(seq(0.75,20.75,1)) - mean(log(seq(0.75,20.75,1)))
ages <- seq(0.01,26,0.1) #10
decentered_log_TotAge <- log(ages) - mean(log(ages))
mean_ber <- ( decentered_log_TotAge - time_ber )/exp(-1*inte_ber)
mean_mat <- ( decentered_log_TotAge - time_mat )/exp(-1*inte_mat)

B <- ns(mean_ber, knots=knot_list , Boundary.knots=boundary_knot_list)
Bm <- ns(mean_mat, knots=knot_list , Boundary.knots=boundary_knot_list)

# plot basis functions
plot( NULL , #xlim=range(mean_ber),
             xlim=boundary_knot_list,
             ylim=c(-1,1),
             xlab="decentered log age" , ylab="basis" )
for ( i in 1:ncol(B) ) lines( mean_ber , B[,i] )

# plot basis functions weighted by coefs
plot( NULL , #xlim=range(mean_ber),
             xlim=boundary_knot_list,
             ylim=c(-50,180),
             xlab="decentered log age" , ylab="basis*coef" )
for ( i in 1:ncol(B) ) lines( mean_ber , B[,i]*coefs[i] )

# plot sum of basis functions weighted by coefs
plot( NULL , #xlim=range(mean_ber),
             xlim=boundary_knot_list,
             ylim=c(-50,180),
             xlab="decentered log age" , ylab="summed (basis*coef)" )
lines( mean_ber , size_ber + ( B %*% coefs[1:10] ) )

# plot unlogged, recentered sum of basis functions weighted by coefs
plot( NULL , #xlim=range(mean_ber),
             xlim=c(0,25),
             ylim=c(0,200),
             xlab="age since conceptions (years)" , ylab="summed (basis*coef)" )
#lines( exp(decentered_log_TotAge_Ber + mean(log(seq(0.75,20.75,1)))) , size_ber + ( B %*% coefs[1:10] ) )
lines( #seq(0.75,20.75,1),
       ages,
       size_ber + ( B %*% coefs[1:10] ), col="red", lwd=2 )
lines( #seq(0.75,20.75,1),
       ages,
       size_mat + ( Bm %*% coefs[1:10] ), col="blue", lwd=2 )

sitar_mal_ber_hts <- cbind( ages, size_ber + ( B %*% coefs[1:10] ) )

# these reconstructed trajectories differ from those using the predict() method below
...
pdf(file="./Plots/sitar_ber_mat.e.pdf",
  ...
  # lines(sitar_model, opt = 'D', subset = eth==0,
  #       lty = BerIndivTraj_lty, lwd=BerIndivTraj_lwd, col=BerIndivTraj_col)
  # lines(sitar_model, opt = 'D', subset = eth==1,
  #       lty = MatIndivTraj_lty, lwd=MatIndivTraj_lwd, col=MatIndivTraj_col)
  ...
  # boundary_knot_list <- c(-2.757394, 1.217558) # copied from SITAR output
  # coefs <- sitar_model$coefficients$fixed
  # ages <- seq(0.01,26,0.1)
  # decentered_log_TotAge <- log(ages) - mean(log(ages))
  # IDeth <- Com.mal.eth.e[match(unique(Com.mal.eth.e$ID), Com.mal.eth.e$ID),c("ID","eth")] # ethnicity for each ID

  # for ( i in 1:nrow(IDeth) ) {

  #   if ( IDeth[i,"eth"] == 0 ) {   # if U.S.

  #     size_ber <- coefs["a"] + random.effects(sitar_model)[IDeth[i,"ID"],"a"]
  #     time_ber <- coefs["b"] + random.effects(sitar_model)[IDeth[i,"ID"],"b"]
  #     inte_ber <- coefs["c"] + random.effects(sitar_model)[IDeth[i,"ID"],"c"]

  #     mean_ber <- ( decentered_log_TotAge - time_ber )/exp(-1*inte_ber)
  #     B <- ns(mean_ber, knots=knot_list , Boundary.knots=boundary_knot_list)

  #     lines( ages, size_ber + ( B %*% coefs[1:10] ), col=BerIndivTraj_col, lwd=BerIndivTraj_lwd, lty=BerIndivTraj_lty )

  #   } else {

  #     size_mat <- coefs["a"] + coefs["a.eth"] + random.effects(sitar_model)[IDeth[i,"ID"],"a"]
  #     time_mat <- coefs["b"] + coefs["b.eth"] + random.effects(sitar_model)[IDeth[i,"ID"],"b"]
  #     inte_mat <- coefs["c"] + coefs["c.eth"] + random.effects(sitar_model)[IDeth[i,"ID"],"c"]

  #     mean_mat <- ( decentered_log_TotAge - time_mat )/exp(-1*inte_mat)
  #     Bm <- ns(mean_mat, knots=knot_list , Boundary.knots=boundary_knot_list)

  #     lines( ages, size_mat + ( Bm %*% coefs[1:10] ), col=MatIndivTraj_col, lwd=MatIndivTraj_lwd, lty=MatIndivTraj_lty )
