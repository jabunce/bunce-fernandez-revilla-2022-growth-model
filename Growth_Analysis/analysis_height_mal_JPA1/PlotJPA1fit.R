...
pdf(file="./Plots/JPA1_Ber_Mat.pdf",
    height=5, width=5)
  ...
  points(x=Ber.mal[which(Ber.mal$TotAge > 0),"TotAge"], y=Ber.mal[which(Ber.mal$TotAge > 0),"Height"], lwd=1, col=BerPoint_col, cex=0.5)
  ...
  post <- post7
  lines(x = x,
        y = 0.012 + mean(post$muA)*exp( -1/( mean(post$muC1)*log( x/mean(post$muD1) + 1 ) + (x/mean(post$muD2))^mean(post$muC2) + (x/mean(post$muD3))^mean(post$muC3) ) ),
