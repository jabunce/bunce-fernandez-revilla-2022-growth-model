...
pdf(file="./Plots/Prior_predict_initial_wf.pdf",
    height=5, width=8)
par(mfrow = c(1, 2))
par(mar = c(0, 0, 0, 0), oma = c(5, 4, 4, 4)) #margins for indiv plot, oma for outer margins (bottom, left, top, right)
  ...
  x2 <- seq(from=(0), to=(26+0.75), by=0.1)            # look at first function all the way back to conception
  ...
  plot( x=0, y=60, type="n", ylim=c(0,80), xlim=c(0,21) ) #, xlab="Age since conception (yrs)", ylab="Height (cm)" )
  ...
  mtext("Weight (cm)", side = 2, outer = T, cex = 1, line = 2, adj=0.5)
  ...
  for ( z in 1:Nsims ){
    ...
    # infant
    lines(x = x2,
          y=( -1*exp(-1*K1[z]*Q1[z]*x2/( 1 + 2*Q1[z] )) + 2*H1[z]/K1[z]*pi^(Q1[z]/( 1 + 2*Q1[z] )) )^(( 1 + 2*Q1[z] )/Q1[z]),
          col="red", lwd=0.25, lty=1)
    

    # child
    lines(x = x,
          y=( -1*exp(-1*K2[z]*Q2[z]*x/( 1 + 2*Q2[z] )) + 2*H2[z]/K2[z]*pi^(Q2[z]/( 1 + 2*Q2[z] )) )^(( 1 + 2*Q2[z] )/Q2[z]),
          col="blue", lwd=0.25, lty=1)

    # adolescent
    lines(x = x,
          y=( -1*exp(-1*K3[z]*Q3[z]*x/( 1 + 2*Q3[z] )) + 2*H3[z]/K3[z]*pi^(Q3[z]/( 1 + 2*Q3[z] )) )^(( 1 + 2*Q3[z] )/Q3[z]),
  ...
  # lines( x=LMSmaleT$Age+0.75, y=LMSmaleT$Mu, col="green", lwd=2 )
  # lines( x=LMSmaleS$Age+0.75, y=LMSmaleS$Mu, col="orange", lwd=2 )
  ...
  for ( z in 1:Nsims ){
    ...
    lines(x = x,
          y=( -1*exp(-1*K1[z]*Q1[z]*x/( 1 + 2*Q1[z] )) + 2*H1[z]/K1[z]*pi^(Q1[z]/( 1 + 2*Q1[z] )) )^(( 1 + 2*Q1[z] )/Q1[z]) +
            ( -1*exp(-1*K2[z]*Q2[z]*x/( 1 + 2*Q2[z] )) + 2*H2[z]/K2[z]*pi^(Q2[z]/( 1 + 2*Q2[z] )) )^(( 1 + 2*Q2[z] )/Q2[z]) +
            ( -1*exp(-1*K3[z]*Q3[z]*x/( 1 + 2*Q3[z] )) + 2*H3[z]/K3[z]*pi^(Q3[z]/( 1 + 2*Q3[z] )) )^(( 1 + 2*Q3[z] )/Q3[z]),
  ...
  # lines( x=LMSmaleT$Age+0.75, y=LMSmaleT$Mu, col="green", lwd=3 )
