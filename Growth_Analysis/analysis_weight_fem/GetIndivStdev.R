...
# data list
data_list2 <- list(
  N = nrow(Ber.fem),                                      # number of weight observations
  J = length(unique(Ber.fem[,"ID"])),                     # number of people measured
  ID = Ber.fem[,"ID"] - ( min( Ber.fem[,"ID"] ) - 1 ),    # vector of individual IDs for each height observation, starting at 1
  age = Ber.fem[,"TotAge"],                               # vector of ages for each observation (in years since conception)
  weight = Ber.fem[,"Weight"],                            # vector of weight observations (in kg)
...
start_list2 <- list(
  sigma = 0.01, # logged stdev for the logged normal likelihood
