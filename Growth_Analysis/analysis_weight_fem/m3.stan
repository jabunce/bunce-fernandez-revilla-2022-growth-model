...
data {
  int<lower=0> N;                           // number of weight observations
  int<lower=0> J;                           // number of people measured
  int<lower=0> E;                           // number of ethnic groups
  array[N] int<lower=0> ID;                 // vector of person IDs for each weight measurement
  vector<lower=0>[N] age;                   // vector of ages for each observation (in years since conception)
  vector<lower=0>[N] weight;                // vector of weight observations (in cm)
  ...
  // mean posteriors for Ber boys from m2
  real<upper=0> LQ1hat;
  real<upper=0> LQ2hat;   
  real<upper=0> LQ3hat;
  real<lower=0> LK1hat;
...
model {
  ...
  for (eth in 1:E) {
    ...
    L_R[eth] ~ lkj_corr_cholesky(pCholEtaR);                   //arrays of cholesky factor 8x8 matrices, lower the eta to allow more extreme correlations, Rethinking pg 394
  ...
  for (n in 1:N) {
    ...
    mu[n] = ( -1*exp(-1*K1[n]*Q1[n]*age[n]/( 1 + 2*Q1[n] )) + 2*H1[n]/K1[n]*pi()^(Q1[n]/( 1 + 2*Q1[n] )) )^(( 1 + 2*Q1[n] )/Q1[n]) +
            ( -1*exp(-1*K2[n]*Q2[n]*age[n]/( 1 + 2*Q2[n] )) + 2*H2[n]/K2[n]*pi()^(Q2[n]/( 1 + 2*Q2[n] )) )^(( 1 + 2*Q2[n] )/Q2[n]) +
            ( -1*exp(-1*K3[n]*Q3[n]*age[n]/( 1 + 2*Q3[n] )) + 2*H3[n]/K3[n]*pi()^(Q3[n]/( 1 + 2*Q3[n] )) )^(( 1 + 2*Q3[n] )/Q3[n]);

  } // for

 
  log_weight ~ normal( log(mu), sigma );
}
...
generated quantities{
  ...
  real berLQ1;                        // parameters for mean ber
  real berLQ2;
  real berLQ3;
  real berLK1;
  real berLK2;
  ...
  real matLQ1;                        // parameters for mean matmane
  real matLQ2;
  real matLQ3;
  real matLK1;
  real matLK2;
  ...
  for (n in 1:N) {
    ...
    log_lik[n] = normal_lpdf( log_weight[n] | log(mu[n]), sigma );            // for WAIC function
