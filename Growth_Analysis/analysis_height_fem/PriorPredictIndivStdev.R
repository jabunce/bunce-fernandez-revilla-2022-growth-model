...
LQ1hat <- mean(post1$muLQ1) #-2.573919
LQ2hat <- mean(post1$muLQ2) #-2.327419        
LQ3hat <- mean(post1$muLQ3) #-10.60936
LK1hat <- mean(post1$muLK1) #3.243073      
LK2hat <- mean(post1$muLK2) #1.27011
LK3hat <- mean(post1$muLK3) #10.34853
R1hat <-  mean(post1$muR1) #5.300416
R2hat <-  mean(post1$muR2) #0.8843116
R3hat <-  mean(post1$muR3) #1.206462  
...
for (n in 1:Nsims) {
  ...
  iexpLQ1 <- 1/0.001 #0.005        # rates for exponential distribution, mean = 1/rate
  iexpLQ2 <- 1/0.005          
  iexpLQ3 <- 1/0.0005 
  iexpLK1 <- 1/0.0005 #0.001       
  iexpLK2 <- 1/0.001 
  iexpLK3 <- 1/0.0001 #0.005
  iexpR1 <-  1/0.03        
  iexpR2 <-  1/0.01
  iexpR3 <-  1/0.03
  ...
  CholEta <- 8
  U <- rlkjcorr( n=1, K=length(offsetIndivStdevs), eta=CholEta )  # eta=1, all coorelations equally likely; eta>1, extreme correlations less likely 
  S <- s %*% U %*% s # vcov matrix with variances (s^2) on the diagonal
