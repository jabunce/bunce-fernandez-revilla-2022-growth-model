...
pdf(file="./Plots/Composite_Ber_m2.pdf",
    height=5, width=5)
  ...
  points(x=Ber.fem$TotAge, y=Ber.fem$Height, lwd=1, col="red", cex=0.1)


  # plot baseline mean trajectory (Shuar). This is the prior.
  post <- data_list2
  ...
  # plot Berkeley posterior mean trajectory
  post <- post2
  ...
  # infant
  x2 <- seq(from=(0), to=(26+0.75), by=0.1)
  ...
  post <- post2
  lines(x = x2,
        y = ( -1*exp(-1*mK1*mQ1*x2/( 1 + 2*mQ1 )) + 2*mH1/mK1 )^(1/mQ1),
        col="black", lwd=3, lty=1)
  ...
  post <- post2
  lines(x = x,
        y = ( -1*exp(-1*mK2*mQ2*x/( 1 + 2*mQ2 )) + 2*mH2/mK2 )^(1/mQ2),
        col="black", lwd=3, lty=1)
  ...
  post <- post2
  lines(x = x,
        y = ( -1*exp(-1*mK3*mQ3*x/( 1 + 2*mQ3 )) + 2*mH3/mK3 )^(1/mQ3),
        col="black", lwd=3, lty=1)
  ...
  par(xpd=TRUE)
  text("Growth velocity (cm/year)", x=32, y=8, srt=270, las=3)

  # reference velocity. This is the prior.
  post <- data_list2
  y_prime <- 1/(1+2*Q1hat)*K1hat*exp((-1*K1hat*Q1hat*x)/(1+2*Q1hat))*( -1*exp((-1*K1hat*Q1hat*x)/(1+2*Q1hat)) + 2*H1hat/K1hat )^(1/Q1hat-1) + 
             1/(1+2*Q2hat)*K2hat*exp((-1*K2hat*Q2hat*x)/(1+2*Q2hat))*( -1*exp((-1*K2hat*Q2hat*x)/(1+2*Q2hat)) + 2*H2hat/K2hat )^(1/Q2hat-1) +
             1/(1+2*Q3hat)*K3hat*exp((-1*K3hat*Q3hat*x)/(1+2*Q3hat))*( -1*exp((-1*K3hat*Q3hat*x)/(1+2*Q3hat)) + 2*H3hat/K3hat )^(1/Q3hat-1)
