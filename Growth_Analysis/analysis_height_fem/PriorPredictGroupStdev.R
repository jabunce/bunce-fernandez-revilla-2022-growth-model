...
for (n in 1:Nsims) {
  ...
  # isLQ1 <- 0.01       # stdevs for normal distribution
  # isLQ2 <- 0.01         
  # isLQ3 <- 0.01 
  # isLK1 <- 0.01       
  # isLK2 <- 0.01 
...
pdf(file="./Plots/Prior_predict_m3_fh.pdf",
    height=10, width=10)
par(mfrow = c(2, 2))
par(mar = c(0, 0, 0, 0), oma = c(5, 5, 4, 4)) #margins for indiv plot, oma for outer margins (bottom, left, top, right)
  ...
  symbol.Female(centerx = 18, centery = 192, rayonx=0.5, lwd=2, col="black") # Female symbol from HelpersMG package
  ...
  symbol.Female(centerx = 20, centery = 192, rayonx=0.5, lwd=2, col="black") # Female symbol from HelpersMG package
