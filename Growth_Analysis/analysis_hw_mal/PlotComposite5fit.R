...
pdf(file="./Plots/Composite5_ber_w.pdf",
  ...
  # plot measurment points
  points(x=Ber.mal$TotAge, y=Ber.mal$CellWeightg, lwd=1, col=grey(0.5), cex=0.5)
  ...
  lines(x = x,
        y = ifelse( x > 0, #weight_offset[ifelse( x == 0, 0, ceiling(x) )] +
                           1.02e-6 + pi*( 2*muH1/muK1 * ( 1 - exp(muK1*muQ1*( 0 - x )/( 1 + 2*muQ1 )) ) )^(1/muQ1 + 2), NA),
        col="black", lwd=3, lty=1)
  ...
  lines(x = x,
        y = ifelse( x > muI1, #weight_offset[ifelse( x == 0, 0, ceiling(x) )] +
                              1.02e-6 + pi*( 2*muH2/muK2 * ( 1 - exp(muK2*muQ2*( muI1 - x )/( 1 + 2*muQ2 )) ) )^(1/muQ2 + 2), NA),
        col="black", lwd=3, lty=1)

  # 3
  lines(x = x,
        y = ifelse( x > muI2, #weight_offset[ifelse( x == 0, 0, ceiling(x) )] +
                              1.02e-6 + pi*( 2*muH3/muK3 * ( 1 - exp(muK3*muQ3*( muI2 - x )/( 1 + 2*muQ3 )) ) )^(1/muQ3 + 2), NA),
        col="black", lwd=3, lty=1)

  # 4
  lines(x = x,
        y = ifelse( x > muI3, #weight_offset[ifelse( x == 0, 0, ceiling(x) )] +
                              1.02e-6 + pi*( 2*muH4/muK4 * ( 1 - exp(muK4*muQ4*( muI3 - x )/( 1 + 2*muQ4 )) ) )^(1/muQ4 + 2), NA),
        col="black", lwd=3, lty=1)

  # 5
  lines(x = x,
        y = ifelse( x > muI4, #weight_offset[ifelse( x == 0, 0, ceiling(x) )] +
                              1.02e-6 + pi*( 2*muH5/muK5 * ( 1 - exp(muK5*muQ5*( muI4 - x )/( 1 + 2*muQ5 )) ) )^(1/muQ5 + 2), NA),
        col="black", lwd=3, lty=1)
...
symbol.Male(centerx = 23, centery = 170, rayonx=0.8, lwd=2, col="black") # Male symbol from HelpersMG package
