...
parameters {
  ...
  real<lower=0,upper=0.005> Lsigma;        // 0.005 variance of the logged normal distribution, upper limit of 0.005 yields 95% of observations within 2*stdev = 1cm of actual height at 100cm  
}
...
model {
  vector[N] mu;                                       // median of un-logged normal distribution
  ...
  offsetGroupStdevs ~ normal(gexpL,0.001); //0.005 0.001 0.005 0.001 0.005 0.01 0.001 0.00001 exponential(1/gexpL);           // hyper-priors for stdevs of group offsets to parameters, on log scale, mean of exponential(beta) = 1/beta
  ...
  for (n in 1:N) {
    ...
    mu[n] = comp_mod( Q[ID[n]], K[ID[n]], H[ID[n]], I[ID[n]], age[n] );
