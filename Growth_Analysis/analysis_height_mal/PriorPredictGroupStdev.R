...
for (n in 1:Nsims) {
  ...
  imLQ1 <- mean(post2$"sigmaInd[1]")#0.005 # 0.0078        # means for normal distribution for stdevs
  imLQ2 <- mean(post2$"sigmaInd[2]")         
  imLQ3 <- mean(post2$"sigmaInd[3]") 
  imLK1 <- mean(post2$"sigmaInd[4]")#0.02 # 0.049 #        
  imLK2 <- mean(post2$"sigmaInd[5]")#0.0004 # 0.00086 #  
  imLK3 <- mean(post2$"sigmaInd[6]")#0.0005 # 0.0013 #  
  imR1 <- mean(post2$"sigmaInd[7]")      
  imR2 <- mean(post2$"sigmaInd[8]")#0.004 # 0.0085 # 
  imR3 <- mean(post2$"sigmaInd[9]")#0.02 # 0.052 # 

  isLQ1 <- 0.001      # stdevs for normal distribution
  isLQ2 <- 0.001         
  isLQ3 <- 0.005 
  isLK1 <- 0.001 #0.005      
  isLK2 <- 0.0001 
...
pdf(file="./Plots/Prior_predict_m3_mh.pdf",
    height=10, width=10)
par(mfrow = c(2, 2))
par(mar = c(0, 0, 0, 0), oma = c(5, 5, 4, 4)) #margins for indiv plot, oma for outer margins (bottom, left, top, right)
  ...
  mtext("Height (cm)", side = 2, outer = T, cex = 1.5, line = 3, adj=-0.28)
  ...
  for ( z in 1:Nsims ){
    ...
    # infant
    lines(x = x2,
          y = ( -1*exp(-1*gK1*gQ1*x2/( 1 + 2*gQ1 )) + 2*gH1/gK1 )^(1/gQ1),
              col="red", lwd=0.25, lty=1)

    # child
    lines(x = x,
          y = ( -1*exp(-1*gK2*gQ2*x/( 1 + 2*gQ2 )) + 2*gH2/gK2 )^(1/gQ2),
          col="blue", lwd=0.25, lty=1)

    # adolescent
    lines(x = x,
          y = ( -1*exp(-1*gK3*gQ3*x/( 1 + 2*gQ3 )) + 2*gH3/gK3 )^(1/gQ3),
  ...
  # infant
  lines(x = x2,
        y=( -1*exp(-1*K1hat*Q1hat*x2/( 1 + 2*Q1hat )) + 2*H1hat/K1hat )^(1/Q1hat),
          col="red4", lwd=3, lty=1)

  # child
  lines(x = x,
        y=( -1*exp(-1*K2hat*Q2hat*x/( 1 + 2*Q2hat )) + 2*H2hat/K2hat )^(1/Q2hat),
          col="darkblue", lwd=3, lty=1)

  # adolescent
  lines(x = x,
        y=( -1*exp(-1*K3hat*Q3hat*x/( 1 + 2*Q3hat )) + 2*H3hat/K3hat )^(1/Q3hat),
          col="darkgreen", lwd=3, lty=1)


  text(1, 190, "Prior group-level variance", cex = 1.35, adj=0)
  symbol.Male(centerx = 18, centery = 190, rayonx=0.5, lwd=2, col="black") # Male symbol from HelpersMG package
  ...
  for ( z in 1:Nsims ){
    ...
    # # composite trajectory
    # lines(x = x,
    #       y = ( -1*exp(-1*K1*Q1*x/( 1 + 2*Q1 )) + 2*H1/K1 )^(1/Q1) +
    #           ( -1*exp(-1*K2*Q2*x/( 1 + 2*Q2 )) + 2*H2/K2 )^(1/Q2) +
    #           ( -1*exp(-1*K3*Q3*x/( 1 + 2*Q3 )) + 2*H3/K3 )^(1/Q3),
    #           col=grey(0.5), lwd=0.25, lty=1)

    # infant
    lines(x = x2,
          y = ( -1*exp(-1*K1*Q1*x2/( 1 + 2*Q1 )) + 2*H1/K1 )^(1/Q1),
              col="red", lwd=0.25, lty=1)

    # child
    lines(x = x,
          y = ( -1*exp(-1*K2*Q2*x/( 1 + 2*Q2 )) + 2*H2/K2 )^(1/Q2),
          col="blue", lwd=0.25, lty=1)

    # adolescent
    lines(x = x,
          y = ( -1*exp(-1*K3*Q3*x/( 1 + 2*Q3 )) + 2*H3/K3 )^(1/Q3),
              col="green", lwd=0.25, lty=1)
  ...
  symbol.Male(centerx = 20, centery = 190, rayonx=0.5, lwd=2, col="black") # Male symbol from HelpersMG package
